...
// Cache regex pattern - avoid recompilation
const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const MIN_PASSWORD_LENGTH = 6;
const SUBMIT_TIMEOUT = 800;
...
export default {
  ...
  beforeUnmount() {
    ...
    if (this.validateEmailTimeout) clearTimeout(this.validateEmailTimeout);
    if (this.validatePasswordTimeout)
      clearTimeout(this.validatePasswordTimeout);
  },
  methods: {
    validateEmail() {
      // Debounce validation - prevent excessive re-renders
    ...
    validatePassword() {
      ...
      this.validatePasswordTimeout = setTimeout(() => {
        if (!this.password) {
          ...
          this.errors.password = `Mật khẩu phải có ít nhất ${MIN_PASSWORD_LENGTH} ký tự.`;
        } else {
